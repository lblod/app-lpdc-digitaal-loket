...
Microservice used to ingest feedback data from IPDC LDES feed, flag instances based on available feedback, and publish
feedback answers back to IPDC.
...
- **processing-status** (via `schema:result`) - Processing state of the feedback (Geaccepteerd/Geweigerd)

## How It Works

### Delta Processing

The service listens for delta notifications from [delta-notifier](https://github.com/mu-semtech/delta-notifier) through
three endpoints:
...
3. **Healing Job** (default: daily at 3 AM `0 3 * * *`)
    ...
    - Corrects incorrectly flagged instances (e.g. when feedback is changed to ipdc-status `INGETROKKEN` and no active
