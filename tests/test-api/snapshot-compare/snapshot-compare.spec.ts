...
test.describe('Compare snapshots', () => {
    ...
    test('When comparing same snapshot then isChanged should be false', async ({request}) => {
        ...
        expect(await compareSnapshots(aSnapshot, aSnapshot, request)).toEqual({isChanged: []});
    });
    ...
    test('When no content change then isChanged should be false', async ({request}) => {
        ...
        expect(await compareSnapshots(currentSnapshot, newSnapshot, request)).toEqual({isChanged: []});
    });
    ...
    test('When content changed then isChanged should be true', async ({request}) => {
        ...
        expect(await compareSnapshots(currentSnapshot, newSnapshot, request)).toEqual({isChanged: ['basisinformatie']});
    });
